...
├── README.md            ← 本文件
├── newAssets/           ← 暂存区：你把美术丢这（详见 newAssets/README.md）
...
- ⚠ **不要直接往这里放中文名文件**——中文名进仓库有跨平台编码风险。中文名只放 `newAssets/`，由导入工具改成 id。

## 工作流（你丢图 → 自动归位 → 游戏显示）

1. 把图标 PNG 丢进 `newAssets/`，**按道具中文名命名**（见 `newAssets/README.md`）。
2. 运行导入工具：
   ```
   <godot> --headless --path . --script res://tools/import_item_art.gd
   ```
   工具会把 `newAssets/臭鸡蛋.png` → `t1_xiangjiaopi.png` 放到本目录，并打印未匹配清单。
