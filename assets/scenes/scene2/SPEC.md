...
| `Waterfall` | 0.18 | **中间偏左通高天河水柱**（`canvas_env_pixel_waterfall`）：当前节点矩形 `(322,-96)-(1042,1120)`。左岸沿 ridge 收紧后从约 55% 高度开始在山体后方隐藏式展开至 164px，不修改左 ridge 素材；右岸保持已确认的顶部轮廓，以 46% / 72% 两个转折形成三段展开，在右 ridge 结束前追到 712px 并保留至少一个屏幕像素格的遮挡重叠，随后到达底部 718px。由此在 ridge 底端可见高度，水体总宽已经超过顶部，形成受重力与触底溅射共同撑开的磅礴水幕。3px 硬像素格内以 8fps 播放 ref26 方法的 8 状态循环。P1-A 将内部组织为贯穿全高的主体、阴影、高光三色面，只让三条分段水带逐帧变化；P1-B 在两岸加入窄幅接触阴影与稀疏断续亮边；P1-C 使用小尺度青白雾舌在水体内部打破 UpperCloud 的水平切割感，同时保持原云形、位置与运动参数 | ✅ P1-A/B/C 水体语言与穿云衔接 |
| `WaterfallRidgeLeft/Right` | 0.16 | 独立的 `scene2_waterfall_ridge_left/right.png` 真透明轮廓山体，在 `Waterfall` 之后绘制，以实体硬边遮住水体两岸；Far/Mid Mountain 与普通远云位于水体后方，不再以半透明色块叠入瀑布内部。保持人工 Position/Size，专属材质使用低雾化深青灰实体色和邻域平均保留克制岩面结构，并仅在各自朝向瀑布的透明硬边加入两级青灰反光；右 ridge 基础内缘使用上缓、中段短肩停顿、下段加速的非线性 1px 小台阶，从画面中线以后另追加最多 8 个源像素的单调退让，使底部最终约退至 x=62px，形成明确宽于上段的瀑布盆口，同时保持上半部不变；随后由程序云幕自然遮挡中段与底部 | ✅ P0-C 实体两岸遮挡 + P2 内缘反光/右岸退让 |
...
| `DistantWater` | 0.54 | 桥后远水面 y=748-910，使用独立 `canvas_env_pixel_distant_water`；4fps、6px/s 的低频横纹与瀑布轴同向的冷色天光通道只在拱洞中露出，左右岸逐步压暗。P2 将窄幅断续落水亮区及慢节奏明暗响应直接并入此 shader，不恢复独立 Impact 节点、不生成扩散环，也不与前景 `River` 争夺动态层级 | ✅ P1 水系统一 + P2 落水环境响应 |
...
- 瀑布运行时顺序为：普通远云与 Far/Mid Mountain → `Waterfall` 实体水柱 → 专属 RidgeLeft/Right 硬遮挡 → 中段与底部程序云幕 → 石桥和河面。该契约消除远山半透明叠入水体的错觉；旧 `WaterfallImpactLeft` 仍保持移除。
...
8. **✅ 瀑布 P2·环境响应**：桥后远水以内建低频亮暗带承接落水；两侧 ridge 只在朝水内缘出现 1–2 档青灰反光；UpperCloud 仅在瀑布轴附近的云底保留一档冷色反光。无 Bloom、无独立冲击叠层，完整战斗画面中角色保持第一焦点、瀑布为第二焦点。

## 自动化自检

通过 Godot Import、Scene2 定向 GUT、节点几何与像素边界探针检查波光运动、遮挡关系和零爬缝；探针不得保存截图。终审由 Eddy F6 完成。
