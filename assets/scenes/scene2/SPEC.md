...
| `BlossomTree` | 0.48 | `scene2_blossom_tree.png` 右侧近中景桃树；整体右移并压低明度/饱和度，深色主枝避开 P2 的头、躯干和武器轮廓，只保留桃粉作为暖色点缀；遮罩把左、上、右末端枝组和下垂小花束分别绕真实连接点做 6fps 小角度像素步进摆动，独立 `scene2_blossom_underpaint.png` 在活动裁片下补出隐藏主干，树根、主干和承重主枝完全静止且不会因裁片位移露出背景 | ✅ P0 战斗静区 + 局部轻风摆动 |
| `PetalFar/Near` | 0.48/1.45 | 共用 `scene2_petal_atlas.png` 的 4 个 16×16 灰粉姿态；远小密、近大疏，以固定 12fps、无插值和随机起始帧形成错拍翻转 | ✅ P2 像素花瓣 |
| `WaterfallLeft` | 0.18 | **中间偏左通高天河水柱**（`canvas_env_pixel_waterfall`）：当前节点矩形 `(388,-124)-(1108,1092)`，视觉轴约 x=748；顶部约 224px、下段约 348px 的宽水体建立磅礴体量。4px 硬像素格内以 5fps / 78px·s⁻¹ 驱动 16px 宽的纵向水带，使用低密度主亮流、克制的缺口和连续大色块取代密集细纹。水体本身连续模拟，最终画面由 V 形山谷和多道云幕切成多个可见窗口 | ✅ 宽幅降噪水体 |
| `WaterfallCloudUpper` | 0.20 | Scene1 同机制的独立像素云丘；复用 Lower 的扁平复合丘形，让相邻云团组合成大小不一的轮廓，再以固定断口避免大空洞、重复单丘和连续长条 | ✅ 中段流云 |
| `WaterfallCloudLower/Lower2` | 0.30 | 两层独立材质的连续云墙：不同种子与相反流向以接近 Scene1 的像素步速平滑漂移；连续云肩消除桥面上方的透明镂空，全宽 U 形包络从瀑布最低点向左山体和右桃树两端逐级升高，峰顶最小宽度为两个像素格 | ✅ 双层瀑底云海 |
...
| `StoneBridge` | 1.0 | `scene2_stone_bridge.png` 作为正式战斗站立层；新源图以有色前景为种子并保留桥石内部阴影，清除深色外底、顶部外露近黑点及桥洞单行/单列提取毛刺后为 `237×55`，Nearest 严格整数放大 7×。P1/P2 落在原图定义的桥面支撑点上；桥本体不挂模糊或调色材质，子节点 `BridgeBankShade` 仅在左右端生成克制的像素接触阴影，并自动跟随石桥变换 | ✅ P1 桥岸衔接 |
...
- 瀑布运行时顺序为：远/中山体 → `WaterfallLeft` 水柱 → 中段与底部程序云幕 → 石桥与河面遮挡 → 前雾。旧 `WaterfallImpactLeft` 已移除，不再附加单独的入水冲击与局部波纹层。
