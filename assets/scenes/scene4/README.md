...
- `FarForest`、`BackgroundTree2`、`BackgroundTree`：保留现有构图，以不同强度的源像素浮雕受光和更分离的鼠标视差建立三档纵深；浮雕只读取相邻原图像素，不平滑像素边缘，也不影响平台、顶部叶或近景树。三层降低统一色盘压缩，避免整片森林像一张同色印刷贴图；`FarForest` 节点保持完全不透明，并继续在下缘渐隐地表线索，避免形成“地面森林”的视觉锚点。
