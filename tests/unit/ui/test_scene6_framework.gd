...
func test_scene6_platform_is_pixel_safe_and_has_local_thermal_depth() -> void:
	...
	assert_not_null(magma)
	if platform == null or atmosphere == null or foreground_embers == null or magma == null:
		return
	...
	if atmosphere_material == null or foreground_material == null \
...
func test_scene6_reuses_character_geometry_with_red_valley_lighting() -> void:
	...
	for node_name: String in ["P1CharDisplay", "P2CharDisplay"]:
		...
		assert_eq(scene6_node.rim_color, Color(1.0, 0.56, 0.24, 1.0))
		assert_lte(scene6_node.rim_strength, 0.12)
		assert_gte(scene6_node.backlight, 0.22)
		assert_eq(scene6_node.shadow_tint, Color(0.46, 0.18, 0.22, 1.0))
		assert_lte(scene6_node.warmth_amount, 0.07)
		assert_eq(scene6_node.fill_color, Color(0.46, 0.13, 0.08, 1.0))
		assert_lte(scene6_node.fill_amount, 0.05)
		...
		assert_lte(material.get_shader_parameter("lava_bounce_amount") as float, 0.10)
		assert_lte(float(material.get_shader_parameter("lava_bounce_start")), 0.50)
		assert_lte(float(material.get_shader_parameter("rim_strength_cap")), 0.30)
	...
	assert_lt(float(p1_material.get_shader_parameter("highlight_compression")),
