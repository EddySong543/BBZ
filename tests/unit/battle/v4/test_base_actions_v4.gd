...
func _battle(e: int = E_INIT, hp: int = HP_INIT) -> BattleEngineV4:
	var p1: Array = []
	var p2: Array = []
	for i in range(3):
...
func _resolve(b: BattleEngineV4, a1: int, a2: int) -> Dictionary:
...
func test_setup_initial_energy_is_one() -> void:
	...
	b.setup(p1, p2, 1)
	# Assert
	assert_eq(b.energy[0], 1, "初始能量 = 1 (ADR-002 D10)")
	assert_eq(b.energy[1], 1)
...
func test_charge_vs_charge_both_gain_energy() -> void:
	var b := _battle()
	_resolve(b, ActionDefV4.Action.CHARGE, ActionDefV4.Action.CHARGE)
	assert_eq(b.energy[0], E_INIT + 1, "攒 +1 能量")
	assert_eq(b.energy[1], E_INIT + 1)
...
func test_charge_vs_attack_p1_takes_one_hp() -> void:
	var b := _battle()
	_resolve(b, ActionDefV4.Action.CHARGE, ActionDefV4.Action.ATTACK)
	assert_eq(b.energy[0], E_INIT + 1)
	assert_eq(b.energy[1], E_INIT - 1, "波 -1 能")
...
func test_attack_vs_attack_both_take_one_hp() -> void:
	...
	var b := _battle()
	_resolve(b, ActionDefV4.Action.ATTACK, ActionDefV4.Action.ATTACK)
	assert_eq(b.energy[0], E_INIT - 1)
	assert_eq(b.energy[1], E_INIT - 1)
...
func test_attack_vs_defend_blocked() -> void:
	var b := _battle()
	_resolve(b, ActionDefV4.Action.ATTACK, ActionDefV4.Action.DEFEND)
	assert_eq(b.energy[0], E_INIT - 1, "波 -1 即使被挡")
	assert_eq(b.energy[1], E_INIT, "防 0 消耗")
...
func test_attack_vs_big_attack_both_take_damage() -> void:
	# B-003 等价：双向都解算，无压制
	var b := _battle()
	_resolve(b, ActionDefV4.Action.ATTACK, ActionDefV4.Action.BIG_ATTACK)
	assert_eq(b.energy[0], E_INIT - 1)
	assert_eq(b.energy[1], E_INIT - 2, "大波 -2 能 (新数值)")
...
func test_attack_vs_big_defend_blocked() -> void:
	var b := _battle()
	_resolve(b, ActionDefV4.Action.ATTACK, ActionDefV4.Action.BIG_DEFEND)
...
func test_big_attack_vs_charge_p2_takes_two_hp() -> void:
	var b := _battle()
	_resolve(b, ActionDefV4.Action.BIG_ATTACK, ActionDefV4.Action.CHARGE)
	assert_eq(b.energy[0], E_INIT - 2, "大波 -2 能 (新数值，v3 为 -3)")
...
func test_big_attack_vs_big_attack_both_take_two_hp() -> void:
	...
	var b := _battle()
	_resolve(b, ActionDefV4.Action.BIG_ATTACK, ActionDefV4.Action.BIG_ATTACK)
	assert_eq(b.energy[0], E_INIT - 2)
...
func test_defend_vs_big_attack_penetrated() -> void:
	# 大波穿防
	var b := _battle()
	_resolve(b, ActionDefV4.Action.DEFEND, ActionDefV4.Action.BIG_ATTACK)
	assert_eq(b.current_hp(0), HP_HALF - BIG, "防被大波穿透，受 2.0")


func test_big_defend_vs_big_attack_blocked() -> void:
	var b := _battle()
	_resolve(b, ActionDefV4.Action.BIG_DEFEND, ActionDefV4.Action.BIG_ATTACK)
...
func test_charge_at_max_energy_does_not_overflow() -> void:
	var b := _battle()
	b.energy = [ActionDefV4.MAX_ENERGY, ActionDefV4.MAX_ENERGY]
	_resolve(b, ActionDefV4.Action.CHARGE, ActionDefV4.Action.CHARGE)
	assert_eq(b.energy[0], ActionDefV4.MAX_ENERGY, "已满攒不溢出")
	assert_eq(b.energy[1], ActionDefV4.MAX_ENERGY)


func test_select_action_rejects_unaffordable() -> void:
	var b := _battle(1, HP_INIT)   # 仅 1 能
	# 大波需 2 能 → 应被拒
	var ok := b.select_action(0, ActionDefV4.Action.BIG_ATTACK)
...
func test_half_point_min_damage_unit() -> void:
	...
	var b := _battle()
	_resolve(b, ActionDefV4.Action.ATTACK, ActionDefV4.Action.CHARGE)
