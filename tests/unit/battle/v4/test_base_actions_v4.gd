...
##   - 大波消耗 3 能（= 6 半能）；能量走半能制（1 能 = 2 半能）；被动 +1 能/回合已恢复（2026-07-03·PASSIVE_ENERGY_GAIN=2）
...
func test_charge_vs_charge_both_gain_energy() -> void:
	...
	assert_eq(b.energy[0], E_INIT + 4, "攒 +1 能(+2 半能) + 被动 +2")
...
func test_charge_vs_attack_p1_takes_one_hp() -> void:
	...
	assert_eq(b.energy[0], E_INIT + 4, "攒 +2 + 被动 +2 半能")
	assert_eq(b.energy[1], E_INIT, "波 -2 + 被动 +2 半能")
...
func test_attack_vs_attack_both_take_one_hp() -> void:
	...
	assert_eq(b.energy[0], E_INIT, "波 -2 + 被动 +2 半能")
...
func test_attack_vs_defend_blocked() -> void:
	...
	assert_eq(b.energy[0], E_INIT, "波 -2 + 被动 +2（即使被挡仍扣）")
	assert_eq(b.energy[1], E_INIT + 2, "防 0 消耗 + 被动 +2")
...
func test_attack_vs_big_attack_both_take_damage() -> void:
	...
	assert_eq(b.energy[0], E_INIT, "波 -2 + 被动 +2 半能")
	assert_eq(b.energy[1], E_INIT - 4, "大波 -6 + 被动 +2 半能")
...
func test_attack_vs_big_defend_blocked() -> void:
	...
	assert_eq(b.energy[1], E_INIT - 2, "大防 -4 + 被动 +2 半能")
...
func test_big_attack_vs_charge_p2_takes_two_hp() -> void:
	...
	assert_eq(b.energy[0], E_INIT - 4, "大波 -6 + 被动 +2 半能")
...
func test_big_defend_vs_big_attack_blocked() -> void:
	...
	assert_eq(b.energy[0], E_INIT - 2, "大防 -4 + 被动 +2 半能")
