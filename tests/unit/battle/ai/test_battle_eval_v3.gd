...
func test_v3_credits_pending_damage_on_opponent() -> void:
	# Arrange：对手出战挂着 1 点延迟伤害（2 半点）
	var b := _neutral()
	var base := BattleEval.score(b, 0)
	b.pending_damage[1][0] = 2

	# Assert：v3 比基础高出 PENDING_W × 2（对手将掉血 = 利好）
	assert_almost_eq(BattleEvalV3.score(b, 0) - base, BattleEvalV3.PENDING_W * 2.0, 0.001,
		"对手头上的延迟伤害应加分")
