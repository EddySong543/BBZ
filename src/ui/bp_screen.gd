...
##           牌库摊开(中·六列固定网格·首发 14=6+6+2·全池一屏无滚动)
...
# ── 牌库网格（六列固定网格·首发 14 = 6+6+2 三行·对手席加高后整体下移 40）──
...
# 末行不足列数则左对齐到列 0（与上方列对齐）。首发 14 = 6+6+2。扩张时按 6 列续行追加即可。
const ROWS: Array = [[0, 6, 602.0], [6, 6, 602.0], [12, 2, 602.0]]
