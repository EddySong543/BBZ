...
# 每行 [起始索引, 卡数, x0]。**6 列锁定（Eddy 2026-06-24·与图鉴一致）**：首发 19 = 6+6+6+1 四行，末行 1 卡左对齐列 0（x0=602=6 卡居中起点·对齐池心 960）。
# 配 ROW_Y0=256 把 4 行居中进池面板（210-874·容得下）；扩张到 24 = 6×4 满四行。
const ROWS: Array = [[0, 6, 602.0], [6, 6, 602.0], [12, 6, 602.0], [18, 1, 602.0]]
