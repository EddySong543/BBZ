...
const GOLD_DEFS: Array = [
	...
	{"id": "screen", "name": "鎏金屏风", "shape": SHAPE_2X3, "gold": 210, "w": 5},
]
const CONSUMABLE_DEFS: Array = [
	{"id": "ration", "name": "干粮", "shape": SHAPE_1X1, "w": 50, "note": "+10 补给"},
	{"id": "potion", "name": "药瓶", "shape": SHAPE_1X1, "w": 30, "note": "单英雄 +1 HP"},
	{"id": "soup", "name": "大补汤", "shape": SHAPE_2X1, "w": 20, "note": "全队 +0.5 HP"},
]
const RARE_DEFS: Array = [
	{"id": "shard", "name": "英雄碎片", "shape": SHAPE_1X1, "w": 50},
	{"id": "skin", "name": "皮肤卷轴", "shape": SHAPE_1X2, "w": 35},
	{"id": "egg", "name": "宠物蛋", "shape": SHAPE_2X2, "w": 15},
]
...
static func make_gold(rng: RandomNumberGenerator) -> Dictionary:
	...
	return {"id": d["id"], "name": d["name"], "cat": "gold", "tier": 0,
		"shape": d["shape"], "gold": int(d["gold"]), "note": "%d金" % int(d["gold"])}
...
static func make_consumable(rng: RandomNumberGenerator) -> Dictionary:
	...
	return {"id": d["id"], "name": d["name"], "cat": "consumable", "tier": 0,
		"shape": d["shape"], "gold": 0, "note": String(d.get("note", ""))}


## 生成一件战斗道具（tier 1-3·形状随 tier 变大）。
static func make_combat(rng: RandomNumberGenerator, tier: int) -> Dictionary:
	...
	if tier == 2:
		...
		shape = SHAPE_2X2
	return {"id": "combat_t%d" % tier, "name": "T%d 战斗道具" % tier, "cat": "combat",
		"tier": tier, "shape": shape, "gold": 0, "note": "可装备或入包带出"}
