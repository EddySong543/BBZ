class_name BattleEngineV4
extends RefCounted

## Battle v4 引擎 —— ADR-002 架构。临时类名 BattleEngineV4，swap 时改回 BattleCore。
...
##
## 组件无状态 (§D2)：所有 per-hero 状态都在本引擎的容器里；HeroSkillV4 只读写传入的 self。

const HP_UNIT := 2  # 必须与 ActionDefV4.HP_UNIT 一致
...
var _skills: Array = [[], []]             # _skills[player][slot]: HeroSkillV4 或 null

## 英雄技能组件注册表：hero_id → 组件脚本。未列入者 = 无技能（_skills 为 null）。
## 随实装逐个加入。swap 后此表与 v3 _HERO_SKILL_SCRIPTS 合并/替换。
const _HERO_SKILL_SCRIPTS := {
	"h01": preload("res://src/battle/v4/skills/h01_qieyun.gd"),
	"h02": preload("res://src/battle/v4/skills/h02_numu.gd"),
	"h03": preload("res://src/battle/v4/skills/h03_kexue.gd"),
	"h04": preload("res://src/battle/v4/skills/h04_sanku.gd"),
	"h05": preload("res://src/battle/v4/skills/h05_tianwei.gd"),
	"h06": preload("res://src/battle/v4/skills/h06_shetui.gd"),
	"h08": preload("res://src/battle/v4/skills/h08_tizui.gd"),
	"h09": preload("res://src/battle/v4/skills/h09_xiongshou.gd"),
	"h10": preload("res://src/battle/v4/skills/h10_tixiao.gd"),
	"h11": preload("res://src/battle/v4/skills/h11_qiongzhui.gd"),
	"h12": preload("res://src/battle/v4/skills/h12_tunshi.gd"),
	"h13": preload("res://src/battle/v4/skills/h13_guzhu.gd"),
	"h15": preload("res://src/battle/v4/skills/h15_sanjian.gd"),
	"h16": preload("res://src/battle/v4/skills/h16_zebei.gd"),
	"h17": preload("res://src/battle/v4/skills/h17_junming.gd"),
	"h20": preload("res://src/battle/v4/skills/h20_qingli.gd"),
	"h32": preload("res://src/battle/v4/skills/h32_chiri.gd"),
	"h18": preload("res://src/battle/v4/skills/h18_jiaohuang.gd"),
	"h19": preload("res://src/battle/v4/skills/h19_lianren.gd"),
	"h21": preload("res://src/battle/v4/skills/h21_xianglong.gd"),
	"h22": preload("res://src/battle/v4/skills/h22_yinzhe.gd"),
	"h23": preload("res://src/battle/v4/skills/h23_zhouerfushi.gd"),
	"h25": preload("res://src/battle/v4/skills/h25_yituiweijin.gd"),
	"h27": preload("res://src/battle/v4/skills/h27_yiroukegang.gd"),
	"h29": preload("res://src/battle/v4/skills/h29_qingchao.gd"),
	"h30": preload("res://src/battle/v4/skills/h30_beichen.gd"),
	"h33": preload("res://src/battle/v4/skills/h33_shenpan.gd"),
	"h34": preload("res://src/battle/v4/skills/h34_huanyu.gd"),
	"h07": preload("res://src/battle/v4/skills/h07_dangxian.gd"),
	"h14": preload("res://src/battle/v4/skills/h14_meikai.gd"),
	"h24": preload("res://src/battle/v4/skills/h24_tianping.gd"),
	"h26": preload("res://src/battle/v4/skills/h26_sishen.gd"),
	"h28": preload("res://src/battle/v4/skills/h28_emo.gd"),
	"h31": preload("res://src/battle/v4/skills/h31_yueliang.gd"),
}
...
func setup(p1_heroes: Array, p2_heroes: Array, seed_value: int = 0) -> void:
	...
	active_index = [0, 0]
	energy = [ActionDefV4.INITIAL_ENERGY, ActionDefV4.INITIAL_ENERGY]
	link = [{}, {}]
	disabled_group = [-1, -1]
...
func _get_cost(player: int, action: int) -> int:
	if action == ActionDefV4.ACTIVE:
		var sk: HeroSkillV4 = _skills[player][active_index[player]]
		return sk.active_cost(self, player, active_index[player]) if sk != null and sk.has_active() else 0
	if action in ActionDefV4.BASE_ACTION_DEF:
		return ActionDefV4.BASE_ACTION_DEF[action]["cost"]
	return 0
...
func is_action_disabled(player: int, action: int) -> bool:
	...
	match disabled_group[player]:
		0:
			return action == ActionDefV4.Action.ATTACK or action == ActionDefV4.Action.BIG_ATTACK
		1:
			return action == ActionDefV4.Action.DEFEND or action == ActionDefV4.Action.BIG_DEFEND
	return false
...
func can_use_active(player: int) -> bool:
	...
	var sk: HeroSkillV4 = _skills[player][slot]
...
func select_active(player: int) -> bool:
	if not can_use_active(player):
		return false
	selected_action[player] = ActionDefV4.ACTIVE
	return true
...
func select_switch(player: int, target_slot: int) -> bool:
	...
	if target_slot == active_index[player] or hp[player][target_slot] <= 0:
		return false
	if not can_afford(player, ActionDefV4.Action.SWITCH):
		return false
	selected_action[player] = ActionDefV4.Action.SWITCH
...
func resolve() -> Dictionary:
	...
	for p in [0, 1]:
		if selected_action[p] < 0:
			push_warning("BattleEngineV4.resolve(): P%d 未选动作，fallback CHARGE" % (p + 1))
			selected_action[p] = ActionDefV4.Action.CHARGE
		elif is_action_disabled(p, selected_action[p]):
			push_warning("BattleEngineV4.resolve(): P%d 选了被禁动作，fallback CHARGE" % (p + 1))
			selected_action[p] = ActionDefV4.Action.CHARGE
	...
	for p in [0, 1]:
		var act: int = a[p]
		var dupable: bool = act == ActionDefV4.Action.CHARGE or act == ActionDefV4.Action.ATTACK \
			or act == ActionDefV4.Action.DEFEND or act == ActionDefV4.Action.BIG_ATTACK \
			or act == ActionDefV4.Action.BIG_DEFEND
	...
	for p in [0, 1]:
		...
		if a[p] == ActionDefV4.Action.CHARGE:
			var gain: int = ActionDefV4.BASE_ACTION_DEF[ActionDefV4.Action.CHARGE]["energy_gain"]
			if dbl[p]:
				gain *= 2   # 梅开二度：攒 ×2
			energy[p] = mini(energy[p] + gain, ActionDefV4.MAX_ENERGY)
			events.append({id = "charge_gain", player = p, amount = gain})
		elif a[p] == ActionDefV4.ACTIVE:
			# 扣能由上面的 _get_cost 完成；cap 计数 + 事件在此；effect 执行延后到 Phase 2.6（切换之后）。
			var slot: int = active_index[p]
			var sk: HeroSkillV4 = _skills[p][slot]
			if sk != null:
	...
	for p in [0, 1]:
		if a[p] == ActionDefV4.Action.SWITCH:
			_do_switch(p, events)

	# Phase 2.6: 即时型主动技执行（在切换之后 → 审判/太阳/三缄 命中对手 post-switch 出战位）
	for p in [0, 1]:
		if a[p] == ActionDefV4.ACTIVE:
			var sk: HeroSkillV4 = _skills[p][active_index[p]]
	...
	for p in [0, 1]:
		...
		for _i in range(hits):
			...
			if a[p] == ActionDefV4.ACTIVE:
				var sk2: HeroSkillV4 = _skills[p][active_index[p]]
	...
	for p in [0, 1]:
		...
		if hp[p][s] > 0:
			var sk: HeroSkillV4 = _skills[p][s]
...
func _perform_switch(player: int, from_slot: int, to_slot: int, events: Array) -> void:
	...
	var leaving: HeroSkillV4 = _skills[player][from_slot]
	if leaving != null:
		leaving.on_switch_out(self, player, from_slot)

	var opp: int = 1 - player
	var opp_active: HeroSkillV4 = _skills[opp][active_index[opp]]
	if opp_active != null:
	...
	var entering: HeroSkillV4 = _skills[player][to_slot]
	if entering != null:
...
func _calc_outgoing(player: int, action: int) -> int:
	var slot: int = active_index[player]
	var dmg := ActionDefV4.get_base_damage(action)
	var skill: HeroSkillV4 = _skills[player][slot]
	...
	for s in range(heroes[player].size()):
		var tsk: HeroSkillV4 = _skills[player][s]
...
func _apply_damage(target_player: int, raw: int, attacker_player: int, atk_action: int, def_action: int, events: Array) -> int:
	...
	if def_action == ActionDefV4.Action.BIG_DEFEND:
		events.append({id = "big_defend_block", player = target_player})
		return 0
	if def_action == ActionDefV4.Action.DEFEND and atk_action == ActionDefV4.Action.ATTACK:
	...
	if dmg > 0:
		for s in range(heroes[target_player].size()):
			if s != slot:
				var bsk: HeroSkillV4 = _skills[target_player][s]
				if bsk != null:
		...
		for p in [0, 1]:
			for slot in range(hp[p].size()):
				if hp[p][slot] > 0 or _death_processed[p][slot]:
					continue
				var skill: HeroSkillV4 = _skills[p][slot]
				...
				if killer >= 0:
					var ks: HeroSkillV4 = _skills[killer][active_index[killer]]
					if ks != null:
				...
				for ally in range(hp[p].size()):
					if ally != slot and hp[p][ally] > 0:
						var ally_skill: HeroSkillV4 = _skills[p][ally]
						if ally_skill != null:
