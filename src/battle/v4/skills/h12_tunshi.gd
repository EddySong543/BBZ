...
func active_attack_damage(_battle: BattleEngineV4, _player: int, _slot: int) -> int:
	return ActionDefV4.get_base_damage(ActionDefV4.Action.ATTACK)   # 1.0

func active_attack_kind() -> int:
	return ActionDefV4.Action.ATTACK   # 被"防"挡

func on_active_attack_resolved(battle: BattleEngineV4, player: int, slot: int, dealt: int) -> void:
	if dealt > 0:
