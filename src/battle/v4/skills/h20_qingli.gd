...
func active_cost(battle: BattleEngineV4, player: int, _slot: int) -> int:
	return battle.energy[player]   # 全部能量

func can_use_active(battle: BattleEngineV4, player: int, _slot: int) -> bool:
...
func active_attack_damage(battle: BattleEngineV4, player: int, _slot: int) -> int:
	# 用回合开始时的能量快照（Phase 2 已扣光当前 energy）
	return battle._energy_before[player] * ActionDefV4.HP_UNIT
