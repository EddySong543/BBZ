...
## h28 恶魔【灵魂缔约】被动 · 开局缔约 · 团队层
## 开局指定一名队友缔约 → 该队友攻击 +1.0；其每次攻击恶魔 -1.0 HP；持续到恶魔或该队友死亡。
...
func modify_team_outgoing_damage(dmg: int, action: int, battle: BattleEngineV4, _attacker_player: int, attacker_slot: int, self_player: int, self_slot: int) -> int:
	if action != ActionDefV4.Action.ATTACK and action != ActionDefV4.Action.BIG_ATTACK:
		return dmg
	...
	if contract >= 0 and attacker_slot == contract and battle.hp[self_player][self_slot] > 0:
		battle.hp[self_player][self_slot] -= ActionDefV4.HP_UNIT
