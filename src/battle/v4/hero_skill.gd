@abstract
class_name HeroSkillV4
extends RefCounted

## 英雄技能组件 v4 基类 (ADR-002 §D2 / §D6)。
## 每个英雄 = src/battle/v4/skills/hXX_*.gd 继承本类、override 所需 hook。
##
## 【组件无状态】：所有 per-hero 运行时状态（窟层/连段/燃烧/变身 form/契约 link…）
##   存在 BattleEngineV4 的容器里（statuses / form / link …）。组件只是纯逻辑，
...
func on_setup(_battle: BattleEngineV4, _player: int, _slot: int) -> void:
	pass
...
func modify_outgoing_damage(dmg: int, _action: int, _battle: BattleEngineV4, _player: int, _slot: int) -> int:
	return dmg
...
func modify_incoming_damage(dmg: int, _action: int, _battle: BattleEngineV4, _player: int, _slot: int, _attacker_player: int) -> int:
	return dmg
...
func modify_team_outgoing_damage(dmg: int, _action: int, _battle: BattleEngineV4, _attacker_player: int, _attacker_slot: int, _self_player: int, _self_slot: int) -> int:
	return dmg
...
func on_ally_take_damage(dmg: int, _ally_slot: int, _battle: BattleEngineV4, _player: int, _slot: int) -> int:
	return dmg


## 本英雄击杀对手某英雄后。overkill = 造成伤害 − 被杀者击杀前剩余 HP（半点，≥0）。
## h03 渴血(团队 +1) / h26 死神(回 2HP) / h29 塔(溢出 splash)。
func on_kill(_victim_player: int, _victim_slot: int, _overkill: int, _battle: BattleEngineV4, _player: int, _slot: int) -> void:
	pass


## 本英雄阵亡时。h03 寅虎(清零团队渴血) / h28 恶魔(解约)。
func on_death(_battle: BattleEngineV4, _player: int, _slot: int) -> void:
	pass


## 队友阵亡时（dead_slot = 同队阵亡者槽位）。
## h19 恋人(挚爱死殉情) / h22 隐者(死亡叠加) / h28(契约目标死解约)。
func on_ally_death(_dead_slot: int, _battle: BattleEngineV4, _player: int, _slot: int) -> void:
	pass


## 致死拦截：本英雄本次将致死时调用。返回 true = 存活（实现内自行改写 HP / 清状态）。
## h06 蛇蜕（首次致死重生 2HP）。默认 false（正常死亡）。
func on_before_death(_battle: BattleEngineV4, _player: int, _slot: int) -> bool:
	return false


## 本英雄登场（开局首发 + 每次切换上场）。h23 周而复始(抽祝福)。
func on_switch_in(_battle: BattleEngineV4, _player: int, _slot: int) -> void:
	pass


## 本英雄切换下场。h04 三窟(+1 窟层)。
func on_switch_out(_battle: BattleEngineV4, _player: int, _slot: int) -> void:
	pass


## 对手出战英雄切换下场时（enemy_slot = 对手下场者槽位）。h11 穷追(追击 1 伤)。
func on_enemy_switch_out(_enemy_slot: int, _battle: BattleEngineV4, _player: int, _slot: int) -> void:
	pass


## 本回合结算末尾（伤害/死亡都处理完后）。h09 凶兽(更新连段) / h25 倒吊人(置蓄势)。
func on_resolve_end(_battle: BattleEngineV4, _player: int, _slot: int) -> void:
	pass


## 回合开始（多数 turn-start 逻辑是引擎级：状态 tick / pending 落地）。预留给需要的英雄。
func on_turn_start(_battle: BattleEngineV4, _player: int, _slot: int) -> void:
	pass
...
## 子类只需补【额外】前置条件。
func can_use_active(_battle: BattleEngineV4, _player: int, _slot: int) -> bool:
	return true


## 执行主动技（即时效果型）。扣能 / cap 计数由引擎处理；此处只写效果。
## 注意：攻击型主动技（active_is_attack()=true）不走本方法，改走下方攻击接口。
func execute_active(_battle: BattleEngineV4, _player: int, _slot: int) -> void:
	pass
...
func active_attack_damage(_battle: BattleEngineV4, _player: int, _slot: int) -> int:
	return 0

## 攻击型主动技在防御门里按哪种基础攻击判定：
##   ATTACK = 被"防"挡；BIG_ATTACK = 穿"防"、被"大防"挡。
func active_attack_kind() -> int:
	return ActionDefV4.Action.ATTACK

## 攻击型主动技命中结算后回调（dealt = 实际落在 HP 上的半点）。吞噬 h12 用于吸血。
func on_active_attack_resolved(_battle: BattleEngineV4, _player: int, _slot: int, _dealt: int) -> void:
	pass
