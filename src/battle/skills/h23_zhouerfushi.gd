...
func _roll(battle: BattleCore, player: int, slot: int) -> void:
	# 重抽前清在场型 buff，避免叠加。
	battle.set_status(player, slot, "wheel_atk", false)
	battle.set_status(player, slot, "wheel_def", false)
	match battle.rng.randi_range(0, 3):
		...
		2:
			battle.shield[player][slot] += ActionDef.HP_UNIT
		3:
			battle.energy[player] = mini(battle.energy[player] + 1, ActionDef.MAX_ENERGY)
