...
##   暗镜 = 明虎 h03【白额雷音】(HP5·负伤后攻击增强·仍可完整防御)
##         → 暗虎【魇镇八极】(厚血 HP7·失控狂暴·把自己的防御一并烧了换穿透·硬穿对手盾)。
##         明=在低血增伤与治疗保命之间取舍，暗=不要命的破坏者（穿防保送·防不掉必中）。
...
##   combo：波穿防 = 保送命中 → 必中地喂全队 on-hit 收割端(蛇叠毒/鸡剑气/猴碎能)；
