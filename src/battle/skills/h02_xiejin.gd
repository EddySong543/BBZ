...
## h02 牛金【磐 · 卸劲】被动 · 防御 · 坦克(HP7 = 14 半点)
## 牛金挨打时，把冲击卸给全队——每名【其他】存活队友获得 0.5HP 护盾（额外血量层）。
##
## 数值（半点制·1.0HP = 2 半点）：
##   护盾 1 层 = SHIELD_LAYER = 1 半点(0.5HP)（对齐房日护甲基准·复用现有 shield 原语）；
...
##     扛得住反打 → 解放整条 combo 线敢打（防御维度的"宽" = 解放整队·非放大某一类伤害）；与房日护甲、护盾道具叠。
...
const SHIELD_LAYER := 1   # 每次卸给队友 1 半点 = 0.5HP 护盾（= 房日护甲基准）
