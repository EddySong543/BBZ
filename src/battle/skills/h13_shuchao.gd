...
##   毒爆(蛇) / 易伤 marked(猎物印记) / 破绽(龙) / 碎能(猴) / 剑意(鸡) / 反震(牛) / 冲撞(马) / 溢杀(暗牛)。
##   纯白板波（无任何附加效果的普通命中）不计 —— 奖励的是【combo 密度】，不是攻击次数。
##   道具提供的额外命中会让 on-hit 派发多次 → 碎能 / 剑意等可各触发多份；龙的破绽改由攻击被挡独立触发。
##
## 设计依据（heroes-redesign / build-design-framework）：
##   维度 = 能量（继承虚日【步虚无有乡】的能量维度·镜维度强制·2026-06-22 Eddy 定）。
##     与步虚无有乡（每次得能 +0.5）/ 纳福（受伤→能）触发面全不同：鼠潮 = combo→能量。
