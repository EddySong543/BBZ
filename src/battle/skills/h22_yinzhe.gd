...
## 每名队友阵亡 → +1 攻 或 减 0.5 受伤，每次阵亡玩家二选一；满编无加成，独存最多 2 点。
##
## ⚠️ atk/def 的"二选一"是玩家决策。当前无 UI → on_ally_death 默认全部计 +攻。
##   def 分配待 UI 接入（届时按玩家选择写 yinzhe_atk / yinzhe_def）。

func on_ally_death(_dead_slot: int, battle: BattleCore, player: int, slot: int) -> void:
...
func modify_outgoing_damage(dmg: int, _action: int, battle: BattleCore, player: int, slot: int) -> int:
	return dmg + int(battle.get_status(player, slot, "yinzhe_atk", 0)) * ActionDef.HP_UNIT


func modify_incoming_damage(dmg: int, _action: int, battle: BattleCore, player: int, slot: int, _attacker_player: int) -> int:
	# 每层 def 减 0.5 受伤（= 1 半点）
	return maxi(dmg - int(battle.get_status(player, slot, "yinzhe_def", 0)), 0)
