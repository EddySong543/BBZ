...
- `design/heroes.md §1` 已收录 **34 个英雄**（12 生肖 + 22 塔罗）全部 8 字段定稿 + 2026-05-24 手记对齐，即将扩到 46（+12 星座）。
- 新数值框架已定（HP 3-7 / 大波 2 能 / 切换 0 能占槽 / 初始 1 能），与现实现的旧数值不同。
- 13 生肖中多数被**推倒重做**（机制全换）。
...
- 架构形态：纯 `RefCounted`、无 UI 依赖、`resolve()` 返回结构化 events、read-only views——这是**联机服务器权威 headless 跑**的前提（ADR-001 D2/D3/D5）。
...
**Rationale**：
- 无状态组件 → 战斗状态全在可序列化容器里 → **联机复制 / 录像重放 / 存档**都只序列化容器，不碰组件实例。这是服务器权威的硬需求。
...
**Rationale**：
- **确定性** > 浮点：服务器权威下，半点整数没有跨平台浮点漂移风险；联机/录像可精确重放。
- 避开浮点等值比较 bug。
...
5. 平减（下限 0，可叠加）：三窟消层 / 教皇 / 力量 / 隐者防
6. 护盾吸收（除非 pierce_shield）
...
**Migration Risks**：相位顺序错 → 减免/护盾/转移/延迟叠加结果错。需 Eddy 签字确认顺序（Open Question Q1）。
...
**Rationale**：统一容器 → 可序列化（联机）；"benched 仍保留的状态"（燃烧/窟/蓄势）有处可存；沉默(h15)只需在容器置 flag，引擎触发被动 hook 前检查跳过。
...
**Rationale**：h34 AOE / h29 溢出是首批多目标连锁死亡场景，必须有确定的触发顺序，否则结果不可预测（联机不同步）。
...
- 新核按 `coding-standards` 先写测试：① 基础动作同时结算矩阵（移植 B-001/2/3 为新数值版）② 伤害管线各相位（减免/护盾/转移/延迟/穿透叠加）③ 每个英雄 2-3 个核心 case ④ 半点 HP / RNG 可复现 / 批量死亡定序。
...
**解锁**：每英雄一个文件、改动局部化；无状态组件 + 半点整数 + seed RNG → 联机服务器权威 / 录像 / 存档具备前提；可组合伤害管线支撑 46 英雄机制多样性。
...
| D2 | 组件持有自身状态（有状态组件） | 难序列化 → 阻碍联机复制 / 存档；改用无状态 + 容器 |
| D3 | float HP | 跨平台浮点漂移威胁服务器权威确定性；半点整数无此问题 |
...
| # | 问题 | 裁定 |
|---|------|------|
| **Q1** | 伤害管线相位顺序（D4） | ✅ **同意**——顺序按逻辑走（转移→护盾→延迟→落 HP）。 |
