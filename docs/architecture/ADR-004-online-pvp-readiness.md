...
> ⏸ **状态覆盖（2026-08-31）**：Frozen。当前产品只推进 PvE 单机，所有 PvP 入口休眠；本文保留为已完成联机地基与未来好友对战的历史参考，不构成当前开发任务。
> 🔶 **状态更新（2026-08-28·道具有序提交）**：每回合提交目标已从下文简写的 `{动作, 道具集}` 升级为客户端私有编排的统一有序步骤。玩家松手释放道具时只在本地记录并播放反馈，最终确认才整段提交；服务器在 BattleCore 克隆上原子校验、权威重放，再广播公开事件流与快照。确认前不向对手逐条泄露。详见 [`2026-08-28-item-sequence-interaction-design.md`](../superpowers/specs/2026-08-28-item-sequence-interaction-design.md)。

| 字段 | 值 |
|------|-----|
| **Status** | ⏸ Frozen（PvE 单机优先；未来好友对战参考） |
| **Date** | 2026-07-12 |
| **Author** | Eddy + Claude |
| **Supersedes** | — |
...
- **M2h 审计三轮（✅ 2026-07-17·外部审计 10 条：7 修·3 条基于旧快照已在 M2f 处理）**：①**ENetTransport 引用环泄漏**——信号 lambda 捕获 self 经 peer 成环（RefCounted 永生·每建房/拨号泄一对）；修=weakref 捕获（救全部路径·close 断信号只救走 close 的）；测=引用归零即释放。②**断线"暂停计时"没真冻结**——deadline 是绝对时间戳·旧行为只是不检查·重连同帧被代提交攒；修=`defer_deadline`（net_session.pump 断链期按实际流逝顺延）。③**中局重连身份门**——原=任何过口令门（公开房无门）连接可顶席位拿快照+控制权；修=重连令牌 `rtk`（Crypto 随机·match_start 只发本人·hello/resync 报到必对上否则 bad_token·client 留底→battle_screen 转存 `BattleSetup.net_rtk` 跨场景→大厅重连带上）。⚠取舍在案：**内存令牌=同进程重连可续·游戏崩溃重开=进不来（重建房便宜·公网化再升持久凭据）**。④**中局重连绕过版本门**——pump 的 hello 门补 gv 校验（与 poll_prestart 对齐）。⑤hello 同队重复英雄拒（旧 LAN 直开局路径的作弊口）。⑥加时骤死补发标准 damage_taken/hero_died 事件（原直写 HP=UI 血条突跳无演出）。⑦远征起点三墙软锁（≈0.8%/局）=确定性凿右邻保底。另：character_display.tscn 过期 UID 改纯路径引用（消存量警告噪音）。验证=GUT 39 脚本 433 用例（+6 回归）+net/lan/net_battle 三探针 PASS（重连段按新契约带令牌）。
- **M2b 服务端工程（剩余）**：进程托管（headless Godot 复用 sim 启动器骨架）+ 公网匹配服务 + 多房间 + 信息扭曲道具的视角假数据渲染（道具真立项时·快照私有字段过滤已在 M2f 落地）+ 公网方案（个人项目候选：Steam P2P / 轻量中转服·需 Eddy 选型·⚠公网化时 DTLS client_unsafe 须升级正经证书或平台通道+重连令牌升持久凭据）。
- **M3 反作弊面收口**：联机构建禁用 src/ui/debug；回合时限服务端计时；提交去重（turn 号幂等）。
- **M4 i18n 抽取（✅ 主体 2026-07-12）**：方案=**「原文即键」**（中文原文即翻译键；未注册翻译表时 `tr()` 原样返回=零行为变化，`.tscn` 文本零改动=Godot 自动翻译按原文查表，Eddy 编辑器所见仍是中文）。已做：①UI 代码显示点全量 `tr()` 包裹（字面量就地包·模板在拼接处包·数据文案在显示汇点包 `tr(变量)`·逻辑键不包）；②键表生成器 `tools/i18n_scan.gd`（可重复跑）→ `assets/i18n/strings_zh.csv`（含英雄 .tres、道具 catalog 与 UI .tscn 文本，带出处 context 列）；③GUT 覆盖翻译边界。**剩余（激活翻译时做）**：填 en 列 → 去 context 列 → 移除 `assets/i18n/.gdignore` → project.godot 注册 translations（风险操作·届时批准）。**明示缓办**：src/expedition（占位内容等 F/G/H 重做）、debug 面板（release 剥离）、标题屏像素字形字（美术资产范畴）。
