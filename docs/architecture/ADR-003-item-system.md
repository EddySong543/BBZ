...
能量经济迁移（B）已完成，半能制 + 大波 3 能 + 被动 +1 能/回合落地——**能量作为「超模总闸门」的前提就绪**（§2 骨架：道具的获取/升级/refill 全吃能量）。道具是本作 build 深度的主体层（§3「浅而广」：6 元件里最多 3 个是道具）；91 件设计全集已定稿，进入实装前必须先锁架构。
...
| **D 同时独立伤害** 🆕扩 | 计算双方**完整出伤 hit-list**（pre 伤害件 → 动作攻击 → post 伤害件，按序）对**快照**算好，再一起施加；每 hit 走 `_apply_damage`（防御门 + 引爆毒 + on-hit） | 沿用 Phase3/4 先快照后施加 → 跨玩家不见彼此 HP 变化；可同时死=平局 |
...
- **道具伤害复用既有管线**（**D1 决策**·`items.md` §1）：命中类道具走 `_apply_damage` → 自动过防御门、按 `Pen` 穿透档、触发 on-hit（引爆毒/喂剑气/碎能）；gate 在「必须穿过防御门连接」（同寅虎，禁分段绕防 proc 脱缰）。
...
- 落点：盲选+同时 → 这些是**预判**（§3A）。结算时：
  - 禁动作/禁切换 → 复用现有 `is_action_disabled` 框架（封印术/定身符 = 道具驱动版 `disabled_group`）。对手若 commit 了被禁动作 → **该动作直接无效**（不执行·无效果·不转攒·能量不扣，Q3 裁定）。⚠ 与现有 h17 君命的 fallback-CHARGE 不一致——h17 保留现状，是否对齐为「无效」留作小后续。
...
- **新测试分层**：① 经济状态机（电报 3 步锁/能量扣费/升级/refill/draft 校验）② 序列结算（pre/post 顺序、动作前+攻 vs 动作后追伤、跨玩家同时不串）③ 道具走防御门/穿透/引爆毒/on-hit（D1/D2 复用正确）④ 七类新能力各 2-3 case ⑤ 半点/seed draft 可复现/clone 零共享。
...
| on-hit 多段 / 多命中 | 连击手套·连环咒·狂舞之刃 | `hit_count` 式 / 多 hit | **复用** |
...
| D3 | 道具走独立结算、不复用 `_apply_damage` | 重复防御门/穿透/on-hit/毒逻辑，D1/D2 难落地、易不一致 |
...
**小后续（非阻塞）**：h17 君命现有 fallback = CHARGE，与道具层「无效」不一致；是否对齐留待实装时定。
