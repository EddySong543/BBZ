...
> 🔶 **状态更新（2026-08-16·普通/稀有/传说连接件扩充）**：当前首发池为 **96 件（T1 31 / T2 39 / T3 26）**，审批全集池为 **145 件**；正式 T2 / T3 目录估值分别统一为 4 / 6；「占卜龟壳」「重卜的龟甲」「命运骰子」「巫毒娃娃」「凶药」已删除。玩家文案与机制映射以 `design/items-firstrelease.md` 和 `src/battle/item_catalog.gd` 为当前真相源。下文早期61/91件架构记录继续仅作溯源。
>
> **当前 T3 状态契约**：一次性效果仍进入 `item_uses`；跨回合效果统一登记到权威 `relics`。同名次数型遗物追加 `charges`，回合型追加 `turns`，唯一型不产生第二个并行乘区；槽中原道具照常在使用后消耗。基础“攻击”只认整次「波 / 大波」，h13 的分段「大波」不重复消费次数或触发整次附加效果。
>
...
> 🔶 **状态更新（2026-07-03·经济重做=免入场税）**：D5 经济状态机的**开格步骤与费率已作废**，Current 行为改为：
...
> 设计真相源见 `design/build-design-framework.md` §2（🔴 2026-07-03 修订）；代码 = `battle_core.gd` 道具经济段。下文 D5 原三步电报/费率作溯源保留。
...
- **二元防御铁则**（[[defense-armor-absorption-model]]）：道具的护盾/护甲 = 额外血量层（`shield`），**禁临时护甲吸收模型**；穿甲 = 无视护甲层但仍受「挡不挡」约束。
...
| **S 前置自结算** 🆕 | 双方各按 `pre_items` 顺序施加**自身向**效果（治疗/护甲/护盾/能量/自 buff/置「本击+X」修正器/置条件 intent）+ 施加**对敌 debuff**（易伤/破防降级/预埋毒/元件层干扰） | 对敌 debuff 只改对手状态、不读对手并发伤害 → 双方先后施加结果对称、无先后手 |
| **A 动作** | 现有 Phase2/2.5/2.6（扣动作能/攒/主动/切换） | 不变 |
| **D 同时独立伤害** 🆕扩 | 计算双方完整伤害列表（pre 伤害件 → 动作攻击 → post 伤害件，按序）对快照算好，再一起施加；独立道具伤害走伤害／护盾管线，但不算命中 | 沿用 Phase3/4 先快照后施加 → 跨玩家不见彼此 HP 变化；可同时死=平局 |
...
- **命中边界**（**D1·2026-08-13 取代旧决策**）：只有基础「波／大波」穿过防御门才算命中；护盾吸收仍算命中。主动技、追击、反击与独立道具伤害只复用伤害／护盾／还魂管线，不引爆毒素、不触发英雄命中技能。
...
- 落点：新标记 `pierce_armor`（hit 上的 bool 或 `Pen` 旁路）；`_apply_damage` **Stage B6 护盾吸收**在 `pierce_armor` 时跳过，但 Stage B4 防御门照常。
...
- **新测试分层**：① 经济状态机（电报 3 步锁/能量扣费/升级/refill/draft 校验）② 序列结算（pre/post 顺序、动作前+攻 vs 动作后追伤、跨玩家同时不串）③ 只有「波／大波」命中引爆毒素／触发英雄技能，独立道具伤害不触发，护盾吸收仍触发 ④ 七类新能力各 2-3 case ⑤ 半点/seed draft 可复现/clone 零共享。
...
| 护甲 / 护盾 / 反弹 | 护盾线·带刺的护盾·应和的钟 | `shield`（额外血量层）+ `on_block` 式反弹 | **复用**（二元铁则） |
...
| D3 | 道具完全绕开统一伤害管线 | 重复护盾、受伤与还魂逻辑；独立道具伤害应复用这些底层结算，但明确不进入命中分支 |
