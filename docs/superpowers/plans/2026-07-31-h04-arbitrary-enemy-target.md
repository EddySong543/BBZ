...
**Architecture:** 在 `HeroSkill` 增加无状态的基础攻击自由选敌 hook；`BattleCore` 为双方各保存本回合基础攻击目标槽，并将该槽写入动作 hit，伤害管线据此结算目标英雄的防御、状态、护盾、受伤、命中与死亡。`legal_actions()` 为 AI 和联机枚举每个存活敌方英雄；本地 UI 复用敌方头像框点选，中央出战位与两个替补位均可选。事件携带目标槽，中央角色只播放自身受击，替补受击反馈落到对应头像框。
...
- 对指定目标使用的防 / 大防仍按敌方本回合动作统一生效；破甲、护盾、毒、印记、易伤、on-hit 与死亡结算都落在实际目标槽。
