...
## 输出：D:/Game/BoBoZan/_probe_output/moon_eclipse_gold.png / moon_eclipse_purple.png（仓库外）
...
func _ready() -> void:
	...
	for v in ["gold", "purple"]:
		...
		await RenderingServer.frame_post_draw
		get_viewport().get_texture().get_image().save_png("D:/Game/BoBoZan/_probe_output/moon_eclipse_%s.png" % v)
