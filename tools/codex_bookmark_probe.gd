...
func _ready() -> void:
	...
	_expect(host.position == Vector2.ZERO, "书页容器未增加左侧空白")
	_expect(host.size == Vector2(1920, 1080), "书页容器保持满屏")
	...
	if _failures.is_empty():
		print("CODEX_BOOKMARK_PROBE_OK: full_screen=1920x1080 cache=ok rarity_jump=ok animation=ok")
		get_tree().quit(0)
	else:
